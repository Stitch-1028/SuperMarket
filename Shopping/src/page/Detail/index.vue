    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :skuImageList="skuInfoList" />
          <!-- 小图列表 -->
          <ImageList :skuImageList="skuInfoList" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(attr, index) in spuSaleAttrList" :key="attr.id">
                ...
                <dd
                  ...
                  @click="checked(attr.spuSaleAttrValueList, value)"
                  changepirce="0"
                  :class="{ active: value.isChecked == 1 }"
                  v-for="(value, indexOf) in attr.spuSaleAttrValueList"
                  :key="value.id"
                >
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <a @click="addToCar(skuInfo.id,skuNum)" href="javascript:">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      checked(spuSaleAttrValueList,value){
        spuSaleAttrValueList.map((item)=>{
          item.isChecked = 0
        })
        value.isChecked = 1
      },
      ...
      async addToCar(skuId,skuNum){
        let obj = {
          skuId,skuNum
        }
        try{

          await this.$store.dispatch('AddToCar',obj)
          window.sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))
          this.$router.push({name:'addToCart',params:{skuNum:skuNum}})
        }catch(err){
        }
